...
Provides different test execution modes for unit tests.
"""
...
def run_unit_tests(test_type="all", verbose=True, markers=None, output_file=None):
    ...
    if test_type == "all":
        ...
        cmd.append("tests/unit/shared/")
    elif test_type == "main":
        cmd.append("tests/unit/test_main.py")
    else:
        cmd.append(f"tests/unit/{test_type}")
...
Examples:
  %(prog)s --type all
  %(prog)s --type shared --markers "not slow"
  %(prog)s --type main --output results.xml
        """
    )
    
    parser.add_argument(
        "--type", 
        choices=["all", "shared", "main"],
        default="all",
